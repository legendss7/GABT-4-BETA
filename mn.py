...
def get_analisis_detalle(df_resultados):
    """Genera un análisis detallado de las fortalezas y debilidades, y el potencial ocupacional.
       LÓGICA ACTUALIZADA PARA MÁS ROLES."""
    ...
    for index, row in top_3.iterrows():
        ...
        aplicacion_clave = get_estrategias_de_mejora(row['Área']).split('**Aplicación:** ')[1].split('.')[0].strip()
        fortalezas_text += f"<li>**{row['Área']} ({row['Percentil']}%)**: Una habilidad sobresaliente en **{row['Área']}** sugiere un alto potencial para **{aplicacion_clave}**.</li>"
    fortalezas_text += "</ul>"
    ...
    for index, row in bottom_3.iterrows():
        ...
        mejoras_text += f"<li>**{row['Área']} ({row['Percentil']}%)**: El desarrollo de **{row['Área']}** debe ser una prioridad, ya que es la base para áreas de procesamiento y detalle. Se sugiere el entrenamiento inmediato en ejercicios de {row['Área'].lower().replace('razonamiento ', '').replace('manual', '').strip()}.</li>"
    mejoras_text += "</ul>"

    # Potencial Ocupacional
    top_area = top_3.iloc[0]['Área']
    
    # Mapeo de Potenciales para roles no aptos (solo usamos las áreas del bottom 3)
    roles_no_aptos = []
    for area in bottom_3['Área']:
        if area in ["Razonamiento General", "Razonamiento Verbal", "Razonamiento Numérico"]:
            roles_no_aptos.append("Roles de Liderazgo Estratégico, Consultoría y Toma de Decisiones (G-Factor)")
        elif area in ["Razonamiento Mecánico", "Razonamiento Espacial", "Razonamiento Técnico"]:
            roles_no_aptos.append("Roles de Ingeniería, Mantenimiento Técnico y Diseño de Productos")
        elif area in ["Razonamiento Clerical", "Razonamiento Abstracto"]:
            roles_no_aptos.append("Roles de Análisis de Datos, Detección de Patrones y Auditoría Documental")
        else: # Velocidad Perceptiva, Precisión Manual, Coordinación Manual, Atención Concentrada
            roles_no_aptos.append("Roles Operativos, de Detalle, Control de Calidad y Ensamblaje Fino")
    
    roles_no_aptos_text = "<ul>" + "".join(f"<li>{rol}</li>" for rol in sorted(list(set(roles_no_aptos)))) + "</ul>"

    # LÓGICA DE POTENCIAL APROBADA (Con nueva categoría Clerical/Abstracto)
    if top_area in ["Razonamiento General", "Razonamiento Verbal", "Razonamiento Numérico"]:
        potencial = "Roles Estratégicos y de Gestión de Información (Consultoría, Finanzas, Liderazgo de Proyectos y Alta Dirección)."
        ...
        potencial = "Roles de Ingeniería, Arquitectura, Diseño Industrial, Mantenimiento Técnico Especializado y Diseño de Producto."
        perfil = "Fuerte Perfil Técnico-Estructural."
    elif top_area in ["Razonamiento Clerical", "Razonamiento Abstracto"]: # NUEVA CATEGORÍA
        potencial = "Roles de Análisis de Datos, Detección de Patrones, Gestión Documental, Investigación y Coordinación Administrativa de Alto Nivel."
        perfil = "Potencial Analítico y Organizativo (R-C Factor)."
    else:
        potencial = "Roles Operativos, de Control de Calidad, Logística, Soporte al Cliente, Operaciones de Detalle y Alto Volumen de Procesamiento."
    ...
    return {
        ...
        "top_area": top_area,
        "roles_no_aptos": roles_no_aptos_text # NUEVO
    }
